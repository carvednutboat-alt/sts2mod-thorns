...
public sealed class SpreadingSporesPower : CustomPowerModel
{
	...
	{
		("title", "´«²¥×ÆºÛ"),
		("description", "Whenever you deal unblocked attack damage, there is a 30% chance to apply 1 [gold]×ÆºÛ[/gold]. While [gold]ÉúÃü»ðÖÖ[/gold] is active, this chance is 100% and can trigger from your unblocked damage.")
	};
}
...
public sealed class LifeSpark : CustomCardModel
{
	...
	{
		("title", "ÉúÃü»ðÖÖ"),
		("description", "Requires [gold]´«²¥×ÆºÛ[/gold]. Gain {LifeSparkPower:diff()} [gold]ÉúÃü»ðÖÖ[/gold]. While active, gain {StrengthPower:diff()} [gold]Strength[/gold], copy your attack damage to another enemy, make [gold]´«²¥×ÆºÛ[/gold] always trigger, and make [gold]×ÆºÛ[/gold] burn enemies for {ScorchHpLoss:diff()} HP. Scorched enemies explode for {ExplosionPercent}% max HP on death. When this ends, remove all [gold]×ÆºÛ[/gold].")
	};
}
