...
internal static class ThornsCharacterIconTexturePatch
{
	...
	{
		if (__instance is Thorns && ResourceLoader.Exists(IconPath))
		{
...
internal static class ThornsCharacterIconOutlineTexturePatch
{
	...
	private static void Postfix(CharacterModel __instance, ref Texture2D __result)
	{
		if (__instance is Thorns && ResourceLoader.Exists(IconOutlinePath))
		{
			__result = ResourceLoader.Load<Texture2D>(IconOutlinePath);
		}
	}
}
...
internal static class ThornsArchitectDefineDialoguesPatch
{
	private static void Postfix(AncientDialogueSet __result)
	{
		...
		if (__result.CharacterDialogues.ContainsKey(thornsKey))
		{
			return;
		}

		__result.CharacterDialogues[thornsKey] = new AncientDialogue[]
		{
			new AncientDialogue("")
			{
				VisitIndex = 0,
